	...
	public class AnchorLayout  implements ILayout
	{
		...
		protected function measureItem(item:DisplayObject, result:Point):void
		{
			...
			if(item is ILayoutDisplayObject)
			{
				...
				if(layoutData)
				{
					maxX = Math.max(maxX, this.measureItemHorizontally(item, layoutData));
					maxY = Math.max(maxY, this.measureItemVertically(item, layoutData));
					isAnchored = true;
				}
			}
			if(!isAnchored)
		 ...
		 * @private
		 */
		protected function measureItemHorizontally(displayItem:DisplayObject, layoutData:AnchorLayoutData):Number
		{
			const left:Number = this.getLeftOffset(displayItem);
			const right:Number = this.getRightOffset(displayItem);
			return displayItem.width + left + right;
		}

		/**
		 * @private
		 */
		protected function measureItemVertically(displayItem:DisplayObject, layoutData:AnchorLayoutData):Number
		{

			const top:Number = this.getTopOffset(displayItem);
			const bottom:Number = this.getBottomOffset(displayItem);
			return displayItem.height + top + bottom;
		}
		...
		protected function layoutVector(items:Vector.<DisplayObject>, unpositionedItems:Vector.<DisplayObject>, boundsX:Number, boundsY:Number, viewPortWidth:Number, viewPortHeight:Number):void
		{
			...
			for(var i:int = 0; i < itemCount; i++)
			{
				...
				this.positionHorizontally(items[i], layoutData, boundsX, boundsY, viewPortWidth, viewPortHeight);
				this.positionVertically(items[i], layoutData, boundsX, boundsY, viewPortWidth, viewPortHeight);
			}
		}
		 ...
		 * @private
		 */
		protected function positionHorizontally(item:DisplayObject, layoutData:AnchorLayoutData, boundsX:Number, boundsY:Number, viewPortWidth:Number, viewPortHeight:Number):void
		{
			const left:Number = layoutData.left;
			const hasLeftPosition:Boolean = !isNaN(left);
			if(hasLeftPosition)
		 ...
		 * @private
		 */
		protected function positionVertically(item:DisplayObject, layoutData:AnchorLayoutData, boundsX:Number, boundsY:Number, viewPortWidth:Number, viewPortHeight:Number):void
		{
			const top:Number = layoutData.top;
			const hasTopPosition:Boolean = !isNaN(top);
			if(hasTopPosition)
