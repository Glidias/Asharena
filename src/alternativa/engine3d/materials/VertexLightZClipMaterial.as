...
package alternativa.engine3d.materials {
	 ...
	 */
	public class VertexLightZClipMaterial extends TextureMaterial {
		...
		public var waterMode:Boolean = false;

		static alternativa3d const _passUVProcedure:Procedure = new Procedure(["#v0=vUV", "#v1=vPosition", "#a0=aUV", "mov v1, i0", "mov v0, a0"], "passUVProcedure");
		...
		/**
		 * Creates a new VertexLightZClipMaterial instance.
		 *
		 * @param diffuse Diffuse map.
		 * @param alpha Transparency.
		 */
		public function VertexLightZClipMaterial(diffuse:TextureResource = null, opacityMap:TextureResource = null, alpha:Number = 1) {
		...
		override public function clone():Material {
			var res:VertexLightZClipMaterial = new VertexLightZClipMaterial(diffuseMap, opacityMap, alpha);
		 ...
		 * @param lightsLength
		 */
		private function getProgram(object:Object3D, programs:Dictionary, camera:Camera3D, materialKey:String, opacityMap:TextureResource, alphaTest:int, lights:Vector.<Light3D>, lightsLength:int, waterMode:Boolean):VertexLightZClipMaterialProgram {
			
			var key:String = materialKey + (opacityMap != null ? "O" : "o") + alphaTest.toString() + (waterMode ? "_water" : "");
			var program:VertexLightZClipMaterialProgram = programs[key];
			if (program == null) {
				...
				vertexLinker.setInputParams(_passUVProcedure, positionVar);
				...
				program = new VertexLightZClipMaterialProgram(vertexLinker, fragmentLinker);
		...
		private function getDrawUnit(program:VertexLightZClipMaterialProgram, camera:Camera3D, surface:Surface, geometry:Geometry, opacityMap:TextureResource, lights:Vector.<Light3D>, lightsLength:int):DrawUnit {
			// Buffers
			...
			drawUnit.setFragmentConstantsFromNumbers(program.cThresholdAlpha, alphaThreshold, 0, waterLevel, alpha);
		...
		override alternativa3d function collectDraws(camera:Camera3D, surface:Surface, geometry:Geometry, lights:Vector.<Light3D>, lightsLength:int, useShadow:Boolean, objectRenderPriority:int = -1):void {
			...
			var program:VertexLightZClipMaterialProgram;
			...
			if (opaquePass && alphaThreshold <= alpha) {
				if (alphaThreshold > 0) {
					...
					program = getProgram(object, optionsPrograms, camera, materialKey, opacityMap, 1, lights, lightsLength, waterMode);
					drawUnit = getDrawUnit(program, camera, surface, geometry, opacityMap, lights, lightsLength);
				} else {
					// do not use opacityMap at all
					program = getProgram(object, optionsPrograms, camera, materialKey, null, 0, lights, lightsLength, waterMode);
			...
			if (transparentPass && alphaThreshold > 0 && alpha > 0) {
				...
				if (alphaThreshold <= alpha && !opaquePass) {
					// Alpha threshold
					program = getProgram(object, optionsPrograms, camera, materialKey, opacityMap, 2, lights, lightsLength, waterMode);
					drawUnit = getDrawUnit(program, camera, surface, geometry, opacityMap, lights, lightsLength);
				} else {
					// There is no Alpha threshold or check z-buffer by previous pass
					program = getProgram(object, optionsPrograms, camera, materialKey, opacityMap, 0, lights, lightsLength, waterMode);
					drawUnit = getDrawUnit(program, camera, surface, geometry, opacityMap, lights, lightsLength);
				}
...
class VertexLightZClipMaterialProgram extends ShaderProgram {
	...
	public function VertexLightZClipMaterialProgram(vertex:Linker, fragment:Linker) {
