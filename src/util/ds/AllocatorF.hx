...
/**
 * Fixed counter buffer allocator. For javascript and Flash targets only. For assinging factory method variant.
 * @author Glidias
 */

class AllocatorF<T> implements haxe.rtti.Generic
{
	private var _method:Void->T;
	private var _i:Int;
	...
	/**
	 * 
	 * @param	method		(Class) A class to instantiate 
	 ...
	 */
	public function new(method:Void->T, fillAmount:Int=0, initialCapacity:Int=0, fixed:Bool=false) 
	{
		_method = method;
		_len = 0;
		this.fixed = fixed;
	...
	inline public function get():T untyped { 
		return _i < _len  ? _vec[_i++] : (_vec[_len++] = _method() );
	}
	...
	public function fill(amount:Int, fixed:Bool):Void {
		...
		while (--amount > -1) {
			if (_vec[amount] == null) _vec[amount] = _method();
		}
		//_vec.fixed = fixed;
	}
